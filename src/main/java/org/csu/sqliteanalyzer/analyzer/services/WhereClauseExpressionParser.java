...
public class WhereClauseExpressionParser {
    ...
    public WhereClauseExpressionParser(List<Map<String, Object>> tokens) {
        try {
            ...
            throw new IllegalArgumentException("Invalid WHERE clause token list", e);
        }
        this.pos = 0;
    ...
    public BooleanExpression parse() throws PlanException {
        try {
            ...
            throw new PlanException("Unable to parse WHERE clause: " + exceptionMessage(e), e);
        }
    }

    private BooleanExpression parseInternal() throws PlanException {
    ...
    private ComparisonExpr parseComparisonExpression() throws PlanException {
        ...
        String leftOperand = parseQualifiedIdentifier();
    ...
    private String parseComparisonOperand() throws PlanException {
        ...
        if ("identifier".equals(currentType())) {
            return sign + parseQualifiedIdentifier();
        }
    ...
    private String parseQualifiedIdentifier() throws PlanException {
        if (!"identifier".equals(currentType())) {
            throw new PlanException(
                    String.format("Expected identifier in WHERE clause, got %s", currentType())
            );
        ...
        while (isCurrentValue(".")) {
            advance();
            if (!"identifier".equals(currentType())) {
                throw new PlanException("Expected identifier after '.' in WHERE clause");
            }
